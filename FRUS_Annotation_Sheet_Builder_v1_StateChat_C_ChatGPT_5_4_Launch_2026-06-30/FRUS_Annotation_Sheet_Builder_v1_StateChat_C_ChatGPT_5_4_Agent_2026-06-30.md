...
- Preserve agency reading-room partial-counterpart status. CIA Reading Room/CREST, agency FOIA libraries, and similar record pages can expose a one-page cover memorandum, detached attachment, or duplicate copy that matches only part of a FRUS printed document. Preserve record-page metadata, PDF access status, and source-record completeness separately, mark `printed_counterpart_match_status: partial_match` when only the wrapper or attachment matches, and request the missing source-image/attached-paper basis instead of backfilling the absent portion from the printed FRUS text.
- Preserve public digital-object status. Bush Library Digital Research Room, National Archives Catalog, presidential-library digital object viewers, and direct storage URLs can expose real source-image PDFs with added citation-marker pages and lossy Adobe Paper Capture text layers. Preserve the digital-object page URL, direct storage PDF URL, item/NAID/local ID metadata, marker-page metadata, PDF creation/modification dates, and OCR confidence separately from the selected document body and FRUS Source note. Do not turn citation-marker metadata into body text or replace a target-volume source-note control number with a storage-path, marker-page folder ID, or partial visible control number.
- Preserve image-only source-folder packet status. Public archival folder scans can contain no embedded text layer, a non-textual citation marker, folder-level withdrawal sheets, unrelated earlier or later records, and several plausible FRUS-selected documents from the same folder. Preserve `image_only_source_pdf_status`, `rendered_page_review_required`, `source_folder_packet_status`, `source_folder_item_inventory`, and `frus_document_to_source_page_map`; do not draft final-looking annotation prose until rendered-page review, OCR/transcription, visible headings, withdrawal rows, FRUS printed-counterpart evidence, or compiler instruction proves the selected document and page range.
- Preserve withdrawn source-record row and body-absent status. Public archival folder PDFs can prove that the selected FRUS source row exists while withholding the selected body image from the public scan. Preserve `withdrawn_source_record_status`, `public_scan_body_absent_status`, `selected_source_row_body_status`, and `printed_transcription_authority`; do not treat the absent body as OCR failure, do not draft from the next visible document, and do not fill handwritten or withdrawn source text from the printed FRUS counterpart unless target-volume or compiler authority supplies that transcription basis.
- Preserve chronological-file packet and forwarding-cover separation. Reagan Library, NSC staff, and other chronological-file PDFs can contain folder covers, withdrawal rows, a staff wrapper, a forwarding memorandum quoted in the FRUS Source note, tab labels, the selected memorandum, and selected or unprinted attachments in one scan. Classify those PDFs as `chronological_file_packet_pdf`, preserve `chron_file_packet_metadata`, `chron_file_item_inventory`, `selected_chron_file_item_status`, `selected_item_tab_or_attachment_status`, and `forwarding_cover_evidence`; do not draft from the whole chron file or turn a forwarding cover into the selected document body.
...
- Preserve presidential-library release and finding-aid packet status. Clinton Library Access Management, FOIA/MDR, Presidential Library, and similar release packets can contain withdrawal/redaction sheets, release-case identifiers, OA/ID rows, folder-title lists, restriction codes, and source-note-like stems without containing the selected document image. Treat those uploads as `source_register_or_finding_aid`; preserve packet/page/row evidence and OCR defects, but do not draft an annotation sheet, document heading, or final source note until the selected document PDF, page range, or compiler authority is supplied.
...
- Preserve serial report packet and item-excerpt status. Recurring evening-report, daily-report, morning-summary, or night-note packets can contain many report dates and several numbered topical items per report, while a FRUS document prints only one item or excerpt. Build a report-date/control-number inventory and a `selected_report_item_scope` before drafting; do not import adjacent report dates, unrelated numbered items, withdrawal rows, or source-copy metadata into the selected item.
...
3. Build a page evidence map:
   ...
   - unreadable pages;
   - withdrawal/redaction rows for selected records whose body pages are absent from the public scan.
...
- If the PDF is a serial packet of recurring reports, build `source_report_series_metadata`, `report_item_inventory`, and `report_item_to_page_map` by report date, control number, signer/office, recipient, numbered item title, page span, classification/handling, and source-copy lane. A packet cover page, withdrawal sheet, or folder title is packet context, not the selected report item.
- If the selected document is only one numbered item or topical excerpt from a longer report, preserve `selected_report_item_scope` and mark whether the compiler selected the item, the whole report, or the whole packet. Do not import other numbered items from the same day, adjacent report dates, or following reports into the selected document's body or source note.
- If the archival packet is a reference or alternate copy whose collection/folder differs from the target-volume Source note, preserve `source_path_divergence_note` and `alternate_copy_or_context_relationship`; do not replace the printed Source note with the packet cover collection, RAC box, withdrawal-sheet ID, or scan folder title unless compiler authority selects that copy.
...
- If evidence is incomplete, return `night_evening_report_basis`, `source_report_series_basis`, `report_item_scope_basis`, `report_packet_item_inventory_basis`, `presidential_read_status_basis`, `report_date_basis`, `drafting_clearance_basis`, `source_path_divergence_basis`, or `document_boundary_basis`.
...
- If an agency reading-room or CREST record exposes only a cover memorandum, detached attachment, record-page body text, or one-page duplicate while the FRUS printed counterpart includes additional attached text, omitted summary, annex, tab, or paper, preserve `agency_reading_room_record_metadata`, `reading_room_pdf_access_status`, `source_record_completeness_status`, and `partial_counterpart_scope_note`. Mark the match as `partial_match` and ask for `source_record_completeness_basis`, `attached_paper_basis`, or `source_image_basis`; do not import the missing attached paper, summary, annex, classification/handling line, or signature from the printed FRUS counterpart.
- If direct web retrieval of an agency PDF is blocked or redirecting but the compiler uploads the PDF, treat the uploaded binary as authoritative and record the public record page/PDF URL as locator evidence only. If only a record page or search-result snippet is supplied, request the actual PDF or source image before drafting body-text claims.
- If a public presidential-library or NARA digital-object PDF includes a non-textual citation-marker page, preserve it as `citation_marker_page_status` / `citation_marker_metadata` and exclude it from selected-document body text. Use marker metadata as locator and release/folder evidence only unless compiler/source authority says it is the selected source-note basis.
- If a direct storage PDF, digital-object viewer, National Archives Catalog item, marker page, and FRUS printed Source note expose different OA/ID, folder ID, local ID, NAID, control number, or visible document number values, preserve `archival_digital_object_metadata`, `digital_object_storage_url`, `source_note_control_number_basis`, and `source_path_divergence_note`. Do not replace the FRUS Source note with the S3/object-viewer path, marker page, or partial visible control number.
- If Adobe Paper Capture or another embedded OCR layer corrupts classification, portion markings, names, page headers, or declassification stamps, preserve `paper_capture_ocr_status` and `text_layer_classification_confidence`. Use rendered source-image review or compiler-supplied transcription for classification/portion-marking claims; do not silently normalize symbols like `~)`, `<T>`, or garbled `SECRE'f'` into final text unless the visible page supports the reading.
...
- If a public archival folder PDF contains only a withdrawal/redaction sheet for the selected row and the selected body pages are absent from the public scan, also classify it as `withdrawn_source_record_counterpart_pdf`. Preserve `withdrawn_source_record_status`, `public_scan_body_absent_status`, `selected_source_row_body_status`, row ID/type/title/date/page count/restriction/classification, adjacent-page evidence, and the `printed_transcription_authority` if a FRUS page or compiler note says an editor transcribed a handwritten source for the volume. The public scan can prove source path and row selection without proving selected body text.
- Do not treat a withdrawn selected source row as OCR failure, a blank document, a source-register-only upload, or a license to draft from the next visible released item. Do not import body text, classification markings, handwriting, participant lists, or speaker turns from the printed FRUS counterpart unless the compiler supplied that counterpart as target-volume authority and the output labels it as printed/transcription authority rather than visible public-scan evidence.
...
- If a public archival folder PDF has no embedded text layer or only form-feed/blank extraction, mark `image_only_source_pdf_status` as image-only or no-text-layer, set `rendered_page_review_required: yes`, and treat OCR/transcription as support evidence rather than an assumed transcript. Do not draft body-text, source-note, classification, or attachment claims from an empty text layer.
- If an image-only source-folder packet contains a citation-marker page, folder-level withdrawal/redaction sheets, unrelated earlier/later records, and multiple plausible selected records, preserve `source_folder_packet_status` and build a `source_folder_item_inventory` before selecting a document. Citation-marker pages and withdrawal-sheet rows are locator/release inventory evidence; they are not selected body text.
- If one archival folder supports more than one printed FRUS document, build a `frus_document_to_source_page_map` that links each printed counterpart URL/document number to the visible or withdrawn source item, page span, heading/date/sender-recipient evidence, and selection confidence. Do not choose Document 219 rather than Document 220, or any other counterpart, merely because both source notes cite the same folder.
- If a public archival PDF is a chronological-file packet with folder-cover pages, withdrawal-sheet rows, staff memoranda, tab labels, and a selected item embedded inside a larger month or chron file, also classify it as `chronological_file_packet_pdf`. Preserve `chron_file_packet_metadata`, `chron_file_item_inventory`, `selected_chron_file_item_status`, `selected_item_tab_or_attachment_status`, `forwarding_cover_evidence`, and `source_note_forwarding_cover_quote`. Use row ID/type/title/date/page count, visible tab labels, heading/date/sender-recipient evidence, FRUS printed-counterpart evidence, and compiler instruction to prove which pages are the selected document, which pages are source-note wrapper evidence, and which attached papers are printed or unprinted.
- Do not turn a chron-file cover page, withdrawal sheet, Tab label, NSC staffing sheet, staff wrapper, or forwarding cover into the selected document body merely because it precedes the selected memorandum. If a forwarding cover supplies Source-note evidence, record it as wrapper evidence and keep its date, drafter, classification, and quoted language separate from the selected memorandum's date, sender/recipient, subject, and attachment status.
- If chron-file row/page/body/counterpart evidence is incomplete, return `blocked_pending_evidence` or request `chron_file_packet_basis`, `chron_file_item_inventory_basis`, `selected_chron_file_item_basis`, `forwarding_cover_basis`, `tab_attachment_basis`, or `selected_document_page_range_basis` before producing final-looking annotation prose.
- If rendered-page review, OCR, withdrawal rows, and printed-counterpart evidence still do not prove the selected document and page range, return `blocked_pending_evidence` or `blocked_pending_ocr_or_rescan` and request `source_folder_packet_basis`, `source_folder_item_inventory_basis`, `frus_document_to_source_page_map_basis`, `image_only_pdf_basis`, `rendered_page_review_basis`, or `selected_document_page_range_basis`. If the selected row is proved but its public body image is absent, request `withdrawn_source_record_basis`, `public_scan_body_absent_basis`, or `printed_transcription_authority_basis` rather than OCR.
...
- If evidence is incomplete, return `archival_record_pdf_basis`, `digitized_source_record_url_basis`, `digital_object_metadata_basis`, `citation_marker_basis`, `text_layer_classification_basis`, `source_note_control_number_basis`, `frus_printed_counterpart_basis`, `printed_counterpart_match_basis`, `alternate_copy_basis`, `source_path_divergence_basis`, `withdrawal_sheet_row_basis`, `folder_document_inventory_basis`, `folder_row_page_map_basis`, `chron_file_packet_basis`, `chron_file_item_inventory_basis`, `selected_chron_file_item_basis`, `forwarding_cover_basis`, `tab_attachment_basis`, `source_folder_packet_basis`, `source_folder_item_inventory_basis`, `frus_document_to_source_page_map_basis`, `image_only_pdf_basis`, `rendered_page_review_basis`, `ocr_text_layer_basis`, `withdrawn_source_record_basis`, `public_scan_body_absent_basis`, `printed_transcription_authority_basis`, `selected_page_range_basis`, or `selected_document_page_range_basis`.
...
- For presidential-library release or finding-aid packets such as Clinton Library Access Management releases, inventory the packet as packet-level context: release or case identifier, part number, PDF page, row number if visible, repository, collection, office or series, OA/ID, folder title, notes column, restriction code, release/withdrawal sheet metadata, and OCR quality. Put the resulting source-note-like wording in `folder_title_source_note_stem` or the evidence ledger, not in `source_note_draft`.
- Use `source_register_packet_metadata`, `source_register_row_inventory`, `source_register_page_locator`, and `source_register_ocr_status` to preserve packet facts. OCR line-wrap artifacts, missing first letters, truncated bracketed folder titles, and merged columns must be marked as transcription limits until a rendered page or human check confirms the row.
...
- Do not infer that a folder listed in a release packet was fully released, digitized, selected for the manuscript, or printed in the target volume. A folder-title row is a lead for retrieval and source-note verification, not a substitute for item-level date, author, recipient, title, classification, attachment, declassification, or document-boundary evidence.
...
- `serial_report_packet_note`: recurring evening-report, daily-report, morning-summary, or night-note packet, report-date/control-number inventory, numbered item scope, selected item versus full-report treatment, adjacent report-date spillover, or report-packet source-copy lane question.
...
- `digital_object_marker_note`: public digital-object/source-image PDF, direct storage URL, Archives Catalog or presidential-library metadata, citation-marker page, Paper Capture OCR status, text-layer/classification confidence, or source-note control-number divergence question.
- `source_folder_packet_note`: image-only public archival source-folder packet, no embedded text layer, rendered-page review requirement, folder-level item inventory, multiple printed FRUS counterparts in one source folder, or selected document/page-range mapping question.
- `withdrawn_source_record_note`: public archival folder PDF, selected withdrawal/redaction row, body pages absent from public scan, row-to-counterpart match, printed transcription authority, or body-absent-versus-OCR-failure question.
- `chron_file_packet_note`: chronological-file packet, month/chron folder cover, withdrawal row, staff wrapper, forwarding memorandum, tab label, selected-item page range, attached-paper treatment, or wrapper-versus-selected-body question.
...
```json
{
  ...
  "document_assessment": {
    ...
    "model": "gpt-5.4",
    "run_mode": "pdf_to_annotation_sheet | batch_pdf_to_annotation_sheet | source_note_only | heading_and_metadata_only | attachment_triage | cover_sheet_triage | volume_or_chapter_packet_triage | source_register_triage | chron_file_packet_triage | publication_placeholder_triage | legacy_digitized_triage | web_print_export_triage | ocr_triage | docx_production",
  ...
  "annotation_sheet_output": {
    ...
    "word_docx_file_name": "",
    "document_count": 0,
    "sheet_status": "first_pass_draft | source_note_incomplete | shorthand_notes_preserved | presidential_update_status_preserved | withdrawn_source_record_body_absent | withheld_in_full_source_note_only | excerpted_body_preserved | compiler_questions_required | volume_or_chapter_packet_no_annotation_sheet | source_register_only_no_annotation_sheet | incremental_placeholder_no_annotation_sheet | legacy_preview_only_no_annotation_sheet | web_export_shell_no_annotation_sheet | cover_sheet_only_no_annotation_sheet | blocked"
  },
