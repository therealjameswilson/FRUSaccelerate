...
16. Run negative-search/no-record validation and usage audit when source notes,
...
17. Run document-relationship validation and usage audit when source notes,
   follow-on footnotes, editorial notes, or attachment notes contain
...
18. Run communications validation and usage audit when source notes, follow-on
...
19. Run source-note and production-marker checks when those unit types are
   present:
...
20. Build the separate evidence queue and General Editor discrepancy ledger:
...
21. Apply safe `comment_only` findings as real Word comments, then apply only
...
22. Reopen and validate the revised `.docx`. Do not release the file if XML,
