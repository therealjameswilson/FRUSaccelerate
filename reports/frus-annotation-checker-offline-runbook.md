...
17. Run negative-search/no-record validation and usage audit when source notes,
...
18. Run document-relationship validation and usage audit when source notes,
   follow-on footnotes, editorial notes, or attachment notes contain
...
19. Run communications validation and usage audit when source notes, follow-on
...
20. Run source-note and production-marker checks when those unit types are
   present:
...
21. Build the separate evidence queue and General Editor discrepancy ledger:
...
22. Apply safe `comment_only` findings as real Word comments, then apply only
...
23. Reopen and validate the revised `.docx`. Do not release the file if XML,
