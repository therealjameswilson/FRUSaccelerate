...
For release/errata apparatus checks, transfer a target-volume registry built
from official status-page entries, volume pages, ebook listings, press
releases, media notes, GPO/ISBN/S/N strings, PDF/EPUB/Mobi download facts,
public URLs, errata corrections, online/full-text correction language, and
printed-volume revision-status fields. The bundled sample is
...
27a. Run release/errata apparatus validation and usage audit when front matter,
   source notes, editorial notes, or tool context mention official volume pages,
   status-page release dates, ebook last-updated dates, EPUB/Mobi/PDF download
   facts, GPO/ISBN/S/N strings, public URLs, press releases, media notes,
   errata corrections, online/full-text corrections, or printed-volume
   revision status. These facts are not source-note provenance. The usage audit
   fails if the model proposes a direct release-apparatus edit unsupported by
   the supplied target-volume release apparatus registry.

```sh
node scripts/validate-frus-release-apparatus-registry.mjs --registry release-apparatus-registry.json --format text
node scripts/audit-frus-release-apparatus-usage.mjs --units extracted-units.json --registry release-apparatus-registry.json --checker-output output.json --target-volume VOLUME-ID --format text
```
...
node scripts/validate-frus-release-apparatus-registry.mjs --registry reports/frus-release-apparatus-registry.sample.json --format text
node scripts/audit-frus-release-apparatus-usage.mjs --units reports/frus-release-apparatus-units.sample.json --registry reports/frus-release-apparatus-registry.sample.json --target-volume frus1989-92v31 --format text
node scripts/test-frus-release-apparatus-audit.mjs
