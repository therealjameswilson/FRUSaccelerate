...
const HANDLING_UNIT_TYPES = new Set([
  "source_note",
  "follow_on_footnote",
  "editorial_note",
  "attachment_note",
...
const HANDLING_PATTERN =
  /\b(initialed|uninitialed|handwritten|wrote|underlined|checkmark|margin|stamped notation|saw it|saw the memorandum|sent for action|sent for information|approved|disapproved|bracketed|brackets are in the original|notation indicates)\b/i;

function usage() {
  console.error(
    "Usage: node scripts/audit-frus-document-handling-usage.mjs --units extracted-units.json --registry registry.json [--checker-output output.json] [--target-volume ENTRY-ID] [--format text|json]"
  );
  process.exit(2);
}
...
function validateRegistry(registry) {
  ...
  if (registry.schema_version !== "frus-document-handling-registry-v1") {
    errors.push("registry.schema_version: must be frus-document-handling-registry-v1");
  }
...
function checkerHandlingDirectEdits(output) {
  const byUnit = new Map();
  if (!output || !Array.isArray(output.checks)) return byUnit;
  for (const check of output.checks) {
    if (!isPlainObject(check)) continue;
    const handlingSignal =
      check.category === "physical_routing_marginalia" ||
      ["physical_evidence_basis", "document_status_basis", "source_image", "wrapper_safety"].includes(check.evidence_request) ||
      /^FAS-HND-\d{3}$/.test(check.rule_id || "");
    if (!handlingSignal || !DIRECT_ACTIONS.has(check.recommended_action)) continue;
...
function usageStatus({ record, match, targetVolume }) {
  if (!String(record.verification_status || "").startsWith(VERIFIED_PREFIX)) return "needs_document_handling_context";
  if (targetVolume && record.volume_id !== targetVolume) return "cross_volume_document_handling";
...
function findingForStatus(status, record, match) {
  if (status === "approved") {
    return `Matched approved document-handling phrase for ${record.document_id} ${record.handling_type}.`;
  }
  if (status === "cross_volume_document_handling") {
    return `Matched document-handling language tied to ${record.volume_id}; confirm target volume before changing initials, stamps, marginalia, routing, copy status, or approval language.`;
  }
  if (status === "needs_document_handling_context") {
    return `Matched ${match.matched_text}, but registry status is ${record.verification_status}; source-image or handling basis is needed before direct edits.`;
  }
  return `Matched a document-handling variant; review against the approved phrase ${JSON.stringify(record.approved_phrase)}.`;
}
...
function isHandlingLikeUnit(unit) {
  return HANDLING_UNIT_TYPES.has(unit.unit_type) && HANDLING_PATTERN.test(unitText(unit));
}
...
function auditDocumentHandling({ unitsDocument, registry, checkerOutput, targetVolume }) {
  const errors = [...validateUnits(unitsDocument), ...validateRegistry(registry), ...validateOutput(checkerOutput)];
  if (errors.length > 0) {
    return {
      schema_version: "frus-document-handling-usage-audit-v1",
      status: "fail",
      errors,
      warnings: [],
      summary: {
        units_scanned: 0,
        document_handling_usages: 0,
        warnings: 0,
        unmatched_document_handling_like_units: 0,
        direct_document_handling_edit_conflicts: 0,
        by_usage_status: {},
        by_handling_type: {}
      },
      usages: [],
  ...
  for (const unit of unitsDocument.units) {
    ...
    if (unitUsages.length === 0 && isHandlingLikeUnit(unit)) {
      unmatched += 1;
      const diagnostic = {
        unit_id: unit.unit_id,
        unit_type: unit.unit_type,
        location: unit.location || "",
        diagnostic_type: "unmatched_document_handling_like_unit",
        finding: "Document-handling or marginalia unit had no match in the supplied document handling registry.",
  ...
  for (const [unitId, checks] of checkerHandlingDirectEdits(checkerOutput).entries()) {
    ...
    for (const check of checks) {
      if (!directEditSupported(check, unitUsages)) {
        directConflicts.push({
          ...
          finding: "Direct document-handling edit lacks a target-volume approved registry match.",
          required_action:
            "Downgrade to comment_only until actor, action, mark location, mark text, routing/decision status, copy status, and source-image basis are supplied."
        });
      }
    }
  }
  ...
  const summary = {
    ...
    document_handling_usages: usages.length,
    warnings: warnings.length,
    unmatched_document_handling_like_units: unmatched,
    direct_document_handling_edit_conflicts: directConflicts.length,
    by_usage_status: countBy(usages.map((item) => item.usage_status)),
    by_handling_type: countBy(usages.map((item) => item.handling_type))
  };
...
function renderText(result) {
  ...
  if (result.status === "fail") {
    lines.push(`FRUS document handling usage audit failed: ${result.errors.length} errors, ${result.warnings.length} warnings.`);
  } else {
    lines.push(
      `FRUS document handling usage audit ${result.status}: ${result.summary.document_handling_usages} matches across ${result.summary.units_scanned} units.`
    );
    lines.push(
      `Variants/cross-volume/context warnings: ${result.summary.warnings}; unmatched document-handling-like units: ${result.summary.unmatched_document_handling_like_units}.`
    );
  }
...
try {
  ...
  const result = auditDocumentHandling({
  ...
  console.error(`FRUS document handling usage audit failed: ${error.message}`);
  process.exit(1);
}
