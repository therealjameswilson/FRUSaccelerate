...
const CONGRESSIONAL_LEGAL_UNIT_TYPES = new Set([
  "source_note",
  "follow_on_footnote",
  "editorial_note",
  "attachment_note",
...
const LEGAL_PATTERN =
  /\b(?:Senate|House|Congress|Congressional|Committee on Foreign Affairs|advice and consent|ratification|transmitted to the Senate|Public Law|P\.L\.|H\.R\.|U\.S\.C\.|Stat\.|Arms Export Control Act|Presidential Determination|Federal Register|appropriations?|authorization|budget authority|budget rescissions|deferrals|formal notice|informal notice|Foreign Military Sales|FMS|IMET|PL 480|Treaty Doc\.|hearings? before|joint session of Congress)\b/i;

function usage() {
  console.error(
    "Usage: node scripts/audit-frus-congressional-legal-usage.mjs --units extracted-units.json --registry registry.json [--checker-output output.json] [--target-volume ENTRY-ID] [--format text|json]"
  );
  process.exit(2);
}
...
function validateRegistry(registry) {
  ...
  if (registry.schema_version !== "frus-congressional-legal-registry-v1") {
    errors.push("registry.schema_version: must be frus-congressional-legal-registry-v1");
  }
...
function checkerCongressionalLegalDirectEdits(output) {
  ...
  for (const check of output.checks) {
    ...
    const legalSignal =
      ["congressional_legal_authority", "treaty_legal_instrument", "economic_financial_data"].includes(check.category) ||
      ["legal_authority", "financial_data", "treaty_component"].includes(check.evidence_request) ||
      /^FAS-(?:CONGRESSIONAL-LEGAL|LEGAL|CL)-\d{3}$/.test(check.rule_id || "") ||
      LEGAL_PATTERN.test(original) ||
      LEGAL_PATTERN.test(replacement);
    if (!legalSignal || !DIRECT_ACTIONS.has(check.recommended_action)) continue;
...
function usageStatus({ record, match, targetVolume }) {
  if (!String(record.verification_status || "").startsWith(VERIFIED_PREFIX)) return "needs_legal_authority_context";
  if (targetVolume && record.volume_id !== targetVolume) return "cross_volume_congressional_legal_context";
...
function findingForStatus(status, record, match) {
  if (status === "approved") {
    return `Matched approved congressional/legal authority phrase for ${record.document_id} ${record.legal_type}.`;
  }
  if (status === "cross_volume_congressional_legal_context") {
    return `Matched congressional/legal authority language tied to ${record.volume_id}; confirm target volume before changing Senate, congressional, statutory, public-law, appropriations, determination, or publication-stage language.`;
  }
  if (status === "needs_legal_authority_context") {
    return `Matched ${match.matched_text}, but registry status is ${record.verification_status}; legal-authority basis is needed before direct edits.`;
  }
  return `Matched a congressional/legal authority variant; review against the approved phrase ${JSON.stringify(record.approved_phrase)}.`;
}
...
function isCongressionalLegalLikeUnit(unit) {
  return CONGRESSIONAL_LEGAL_UNIT_TYPES.has(unit.unit_type) && LEGAL_PATTERN.test(unitText(unit));
}
...
function auditCongressionalLegal({ unitsDocument, registry, checkerOutput, targetVolume }) {
  const errors = [...validateUnits(unitsDocument), ...validateRegistry(registry), ...validateOutput(checkerOutput)];
  if (errors.length > 0) {
    return {
      schema_version: "frus-congressional-legal-usage-audit-v1",
      status: "fail",
      errors,
      warnings: [],
      summary: {
        units_scanned: 0,
        congressional_legal_usages: 0,
        warnings: 0,
        unmatched_congressional_legal_like_units: 0,
        direct_congressional_legal_edit_conflicts: 0,
        by_usage_status: {},
        by_legal_type: {}
      },
      usages: [],
  ...
  for (const unit of unitsDocument.units) {
    ...
    if (unitUsages.length === 0 && isCongressionalLegalLikeUnit(unit)) {
      unmatched += 1;
      const diagnostic = {
        unit_id: unit.unit_id,
        unit_type: unit.unit_type,
        location: unit.location || "",
        diagnostic_type: "unmatched_congressional_legal_like_unit",
        finding:
          "Congressional, statutory, public-law, Senate, budget, appropriations, determination, or Federal Register language had no match in the supplied congressional/legal registry.",
  ...
  for (const [unitId, checks] of checkerCongressionalLegalDirectEdits(checkerOutput).entries()) {
    ...
    for (const check of checks) {
      if (!directEditSupported(check, unitUsages)) {
        directConflicts.push({
          ...
          finding: "Direct congressional/legal authority edit lacks a target-volume approved registry match.",
          required_action:
            "Downgrade to comment_only until the committee, hearing, public-law, statute, Presidential Determination, advice-and-consent status, budget/appropriation figure, or publication-stage basis is supplied."
        });
      }
    }
  }
  ...
  const summary = {
    ...
    congressional_legal_usages: usages.length,
    warnings: warnings.length,
    unmatched_congressional_legal_like_units: unmatched,
    direct_congressional_legal_edit_conflicts: directConflicts.length,
    by_usage_status: countBy(usages.map((item) => item.usage_status)),
    by_legal_type: countBy(usages.map((item) => item.legal_type))
  };
...
function renderText(result) {
  ...
  if (result.status === "fail") {
    lines.push(`FRUS congressional/legal usage audit failed: ${result.errors.length} errors, ${result.warnings.length} warnings.`);
  } else {
    lines.push(`FRUS congressional/legal usage audit ${result.status}: ${result.summary.congressional_legal_usages} matches across ${result.summary.units_scanned} units.`);
    lines.push(`Variants/cross-volume/context warnings: ${result.summary.warnings}; unmatched congressional/legal-like units: ${result.summary.unmatched_congressional_legal_like_units}.`);
  }
...
try {
  ...
  const result = auditCongressionalLegal({
  ...
  console.error(`FRUS congressional/legal usage audit failed: ${error.message}`);
  process.exit(1);
}
