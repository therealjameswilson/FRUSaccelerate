...
const TREATY_UNIT_TYPES = new Set([
  "source_note",
  "follow_on_footnote",
  "editorial_note",
  "document_heading",
...
function usage() {
  console.error(
    "Usage: node scripts/audit-frus-treaty-usage.mjs --units extracted-units.json --registry registry.json [--checker-output output.json] [--target-volume ENTRY-ID] [--format text|json]"
  );
  process.exit(2);
}
...
function validateRegistry(registry) {
  ...
  if (registry.schema_version !== "frus-treaty-registry-v1") {
    errors.push("registry.schema_version: must be frus-treaty-registry-v1");
  }
...
function checkerTreatyDirectEdits(output) {
...
function usageStatus({ record, match, targetVolume }) {
  if (!String(record.verification_status || "").startsWith(VERIFIED_PREFIX)) return "needs_treaty_context";
  if (targetVolume && record.volume_id !== targetVolume) return "cross_volume_treaty";
...
function findingForStatus(status, record, match) {
  ...
  return `Matched a treaty/legal variant; review against the approved phrase ${JSON.stringify(record.approved_phrase)}.`;
}
...
function isTreatyLikeUnit(unit) {
  return TREATY_UNIT_TYPES.has(unit.unit_type) && TREATY_PATTERN.test(unitText(unit));
}
...
function auditTreaties({ unitsDocument, registry, checkerOutput, targetVolume }) {
  const errors = [...validateUnits(unitsDocument), ...validateRegistry(registry), ...validateOutput(checkerOutput)];
  if (errors.length > 0) {
    return {
      schema_version: "frus-treaty-usage-audit-v1",
      status: "fail",
      errors,
      warnings: [],
      summary: {
        ...
        treaty_usages: 0,
        warnings: 0,
        unmatched_treaty_like_units: 0,
        direct_treaty_edit_conflicts: 0,
        by_usage_status: {},
        by_treaty_component_type: {}
      },
      usages: [],
      diagnostics: [],
  ...
  for (const unit of unitsDocument.units) {
    ...
    if (unitUsages.length === 0 && isTreatyLikeUnit(unit)) {
      unmatched += 1;
      const diagnostic = {
        unit_id: unit.unit_id,
        unit_type: unit.unit_type,
  ...
  for (const [unitId, checks] of checkerTreatyDirectEdits(checkerOutput).entries()) {
  ...
  const summary = {
    ...
    treaty_usages: usages.length,
    warnings: warnings.length,
    unmatched_treaty_like_units: unmatched,
    direct_treaty_edit_conflicts: directConflicts.length,
    by_usage_status: countBy(usages.map((item) => item.usage_status)),
    by_treaty_component_type: countBy(usages.map((item) => item.treaty_component_type))
  };
...
function renderText(result) {
  ...
  if (result.status === "fail") {
    lines.push(`FRUS treaty usage audit failed: ${result.errors.length} errors, ${result.warnings.length} warnings.`);
  } else {
    lines.push(`FRUS treaty usage audit ${result.status}: ${result.summary.treaty_usages} matches across ${result.summary.units_scanned} units.`);
    lines.push(`Variants/cross-volume/context warnings: ${result.summary.warnings}; unmatched treaty-like units: ${result.summary.unmatched_treaty_like_units}.`);
  }
...
try {
  ...
  console.error(`FRUS treaty usage audit failed: ${error.message}`);
  process.exit(1);
}
