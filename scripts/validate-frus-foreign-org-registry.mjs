...
const SCHEMA_VERSION = "frus-foreign-org-registry-v1";
...
const ALLOWED_STATUSES = new Set([
  "verified_published_foreign_org_record",
  "verified_local_foreign_org_record",
  "needs_entity_identity_basis",
  "needs_successor_state_basis",
  "needs_treaty_party_basis",
  "needs_translation_basis",
  "unknown"
]);

function usage() {
  console.error("Usage: node scripts/validate-frus-foreign-org-registry.mjs --registry registry.json [--format text|json]");
  process.exit(2);
}
...
function validateRegistry(registry) {
  ...
  for (const key of ["foreign_org_registry_id", "captured_at", "scope"]) {
  ...
  registry.records.forEach((record, index) => {
    ...
    if (typeof record.foreign_org_id === "string") {
      if (seen.has(record.foreign_org_id)) errors.push(`${label}.foreign_org_id: duplicate ${record.foreign_org_id}`);
      seen.add(record.foreign_org_id);
    }
    if (typeof record.entity_type === "string" && !ALLOWED_ENTITY_TYPES.has(record.entity_type)) {
      warnings.push(`${label}.entity_type: unknown value ${record.entity_type}`);
    }
...
function resultFor(registryPath, registry) {
  ...
  return {
    schema_version: "frus-foreign-org-registry-validation-v1",
...
try {
  ...
  if (options.format === "json") {
    ...
    console.log(`FRUS foreign/international-organization registry validation failed: ${result.errors.length} errors, ${result.warnings.length} warnings.`);
    for (const error of result.errors) console.log(`- ${error}`);
    for (const warning of result.warnings) console.log(`warning: ${warning}`);
  } else {
    console.log(`FRUS foreign/international-organization registry validation ${result.status}: ${result.summary.records} records, ${result.summary.warnings} warnings.`);
    for (const warning of result.warnings) console.log(`warning: ${warning}`);
  }
  process.exit(result.status === "fail" ? 1 : 0);
} catch (error) {
  console.error(`FRUS foreign/international-organization registry validation failed: ${error.message}`);
  process.exit(1);
}
