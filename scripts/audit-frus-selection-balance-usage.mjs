...
const SELECTION_UNIT_TYPES = new Set([
  "source_note",
  "follow_on_footnote",
  "editorial_note",
  "front_matter",
...
const SELECTION_PATTERN =
  /\b(?:complete(?:\s+(?:memorandum|record|coverage|documentation|selection))?|balanced|representative|selection|selected|scope|omitted here|not related|scheduled for publication|printed in Foreign Relations|publication in Foreign Relations|related volume|closely linked|known gap|withheld-document|outside (?:the )?scope|all major|no other (?:significant )?(?:records|documentation)|nothing else)\b/i;

function usage() {
  console.error(
    "Usage: node scripts/audit-frus-selection-balance-usage.mjs --units extracted-units.json --registry registry.json [--checker-output output.json] [--target-volume ENTRY-ID] [--format text|json]"
  );
  process.exit(2);
}
...
function validateRegistry(registry) {
  ...
  if (registry.schema_version !== "frus-selection-balance-registry-v1") {
    errors.push("registry.schema_version: must be frus-selection-balance-registry-v1");
  }
...
function usageStatus({ record, match, targetVolume }) {
  if (!String(record.verification_status || "").startsWith(VERIFIED_PREFIX)) return "needs_selection_balance_context";
  if (targetVolume && record.volume_id !== targetVolume) return "cross_volume_selection_context";
...
function findingForStatus(status, record, match) {
  if (status === "approved") {
    return `Matched approved selection/scope phrase for ${record.document_id} ${record.selection_issue_type}.`;
  }
  if (status === "cross_volume_selection_context") {
    return `Matched selection/scope language tied to ${record.volume_id}; use it only as a style model until the target volume's selection-balance context proves the scope claim.`;
  }
  if (status === "needs_selection_balance_context") {
    return `Matched ${match.matched_text}, but registry status is ${record.verification_status}; selection-balance evidence is needed before final-style claims.`;
  }
  return `Matched a selection/scope variant; review against the approved phrase ${JSON.stringify(record.approved_phrase)}.`;
}
...
function isSelectionLikeUnit(unit) {
  return SELECTION_UNIT_TYPES.has(unit.unit_type) && SELECTION_PATTERN.test(unitText(unit));
}

function checkerSelectionDirectEdits(output) {
  ...
  return output.checks.filter((check) => {
    ...
    return (
      check.category === "selection_balance_completeness" ||
      check.evidence_request === "selection_balance_basis" ||
      /^FAS-SEL-\d{3}$/.test(check.rule_id || "") ||
      SELECTION_PATTERN.test(`${check.original_text || ""}\n${check.replacement_text || ""}`)
    );
  });
}

function directEditConflicts(output) {
  return checkerSelectionDirectEdits(output).map((check) => ({
    unit_id: check.unit_id || "",
    rule_id: check.rule_id || "",
    original_text: check.original_text || "",
    replacement_text: check.replacement_text || "",
    finding: "Direct edit touches a selection, balance, completeness, excerpt, or related-volume scope claim.",
    required_action:
      "Downgrade to comment_only. Selection-balance and completeness claims require target-volume evidence and General Editor review before final apparatus."
  }));
}

function auditSelectionBalance({ unitsDocument, registry, checkerOutput, targetVolume }) {
  const errors = [...validateUnits(unitsDocument), ...validateRegistry(registry), ...validateOutput(checkerOutput)];
  if (errors.length > 0) {
    return {
      schema_version: "frus-selection-balance-usage-audit-v1",
      status: "fail",
      ...
      summary: {
        units_scanned: 0,
        selection_usages: 0,
        unmatched_selection_like_units: 0,
        direct_selection_edit_conflicts: 0,
        warnings: 0,
        by_usage_status: {},
        by_selection_issue_type: {}
      },
      usages: [],
      unmatched_selection_like_units: [],
  ...
  const unmatched = (unitsDocument.units || [])
    .filter((unit) => !matchedUnits.has(unit.unit_id) && isSelectionLikeUnit(unit))
    .map((unit) => ({
      unit_id: unit.unit_id,
      unit_type: unit.unit_type,
      location: unit.location || "",
      matched_text: unitText(unit).match(SELECTION_PATTERN)?.[0] || "",
      finding: "Selection, balance, completeness, excerpt, omission, or related-volume language had no match in the supplied selection-balance registry.",
      required_action:
        "Request selection-balance evidence or keep the issue in comment-only review; do not accept complete/balanced/no-other-record claims on wording alone.",
  ...
  const summary = {
    ...
    selection_usages: usages.length,
    unmatched_selection_like_units: unmatched.length,
    direct_selection_edit_conflicts: conflicts.length,
    warnings: warnings.length,
    by_usage_status: countBy(usages.map((usage) => usage.usage_status)),
    by_selection_issue_type: countBy(usages.map((usage) => usage.selection_issue_type))
  };
  return {
    schema_version: "frus-selection-balance-usage-audit-v1",
    ...
    summary,
    usages,
    unmatched_selection_like_units: unmatched,
...
function renderText(result) {
  ...
  if (result.status === "fail") {
    lines.push(`FRUS selection-balance usage audit failed: ${result.errors.length} errors, ${result.warnings.length} warnings.`);
  } else {
    lines.push(
      `FRUS selection-balance usage audit ${result.status}: ${result.summary.selection_usages} usages, ${result.summary.unmatched_selection_like_units} unmatched selection-like units.`
    );
  }
...
try {
  ...
  const result = auditSelectionBalance({
  ...
  console.error(`FRUS selection-balance usage audit failed: ${error.message}`);
  process.exit(1);
}
