...
function compactSourceFamilyRegistry(registry, targetVolume) {
  ...
  return {
    ...
    scope: registry.scope || "",
    target_volume: targetVolume,
    target_families: targetVolume ? families.filter((family) => family.volume_id === targetVolume) : [],
...
function compactDocumentStatusLifecycleRegistry(registry, targetVolume) {
  if (!registry) return null;
  const records = Array.isArray(registry.records) ? registry.records : [];
  return {
    schema_version: registry.schema_version,
    document_status_lifecycle_registry_id: registry.document_status_lifecycle_registry_id,
    captured_at: registry.captured_at,
    source_urls: registry.source_urls || [],
    scope: registry.scope || "",
    target_volume: targetVolume,
    target_records: targetVolume ? records.filter((record) => record.volume_id === targetVolume) : [],
    records: records.map((record) => ({
      document_status_item_id: record.document_status_item_id,
      volume_id: record.volume_id,
      document_id: record.document_id,
      document_number: record.document_number,
      unit_scope: record.unit_scope,
      ...
      approved_phrase: record.approved_phrase,
      variant_forms: record.variant_forms || [],
      source_note_component: record.source_note_component,
      copy_or_version_status: record.copy_or_version_status,
      drafting_or_clearance_basis: record.drafting_or_clearance_basis,
      routing_or_distribution_status: record.routing_or_distribution_status,
      read_or_approval_status: record.read_or_approval_status,
      caveat_or_limitation: record.caveat_or_limitation,
      source_or_context: record.source_or_context,
      source_url: record.source_url,
      verification_status: record.verification_status
...
function renderPacket({
  ...
  return [
    ...
    "",
    "Use these deterministic wrapper-extracted publication-status phrases to avoid silently missing status drift. They are not provenance. If a claim has only a volume target, do not propose a direct status redline.",
    "",
    ...
    "",
    fencedJson(sourceFamilyRegistry || {}),
    "",
    ...
    "",
    fencedJson(documentStatusLifecycleRegistry || {}),
    "",
    ...
    "",
    fencedJson(editorialMethodRegistry || {}),
    "",
    ...
    "",
    fencedJson(crossReferenceRegistry || {}),
    "",
...
function buildChunks(options) {
  ...
  const sourceFamilyRegistry = options.sourceFamilyRegistryPath ? readJson(options.sourceFamilyRegistryPath) : null;
  const sourceSurrogateRegistry = options.sourceSurrogateRegistryPath ? readJson(options.sourceSurrogateRegistryPath) : null;
  const documentStatusLifecycleRegistry = options.documentStatusLifecycleRegistryPath
    ? readJson(options.documentStatusLifecycleRegistryPath)
    : null;
  ...
  const editorialMethodRegistry = options.editorialMethodRegistryPath
    ? readJson(options.editorialMethodRegistryPath)
    : null;
  ...
  const footnoteReferbackRegistry = options.footnoteReferbackRegistryPath
    ...
    : null;
  const crossReferenceRegistry = options.crossReferenceRegistryPath ? readJson(options.crossReferenceRegistryPath) : null;
  ...
  const sourceFamilyRegistryContext = compactSourceFamilyRegistry(sourceFamilyRegistry, options.targetVolume);
  const sourceSurrogateRegistryContext = compactSourceSurrogateRegistry(sourceSurrogateRegistry, options.targetVolume);
  const documentStatusLifecycleRegistryContext = compactDocumentStatusLifecycleRegistry(
    documentStatusLifecycleRegistry,
  ...
  const editorialMethodRegistryContext = compactEditorialMethodRegistry(editorialMethodRegistry, options.targetVolume);
  ...
  const crossReferenceRegistryContext = compactCrossReferenceRegistry(crossReferenceRegistry, options.targetVolume);
  ...
  unitChunks.forEach((units, index) => {
    ...
    fs.writeFileSync(
      ...
      renderPacket({
        ...
        sourceFamilyRegistry: sourceFamilyRegistryContext,
        sourceSurrogateRegistry: sourceSurrogateRegistryContext,
        documentStatusLifecycleRegistry: documentStatusLifecycleRegistryContext,
        ...
        editorialMethodRegistry: editorialMethodRegistryContext,
        ...
        crossReferenceRegistry: crossReferenceRegistryContext,
