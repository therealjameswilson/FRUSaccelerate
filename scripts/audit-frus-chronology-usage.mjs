...
const CHRONOLOGY_UNIT_TYPES = new Set([
  "source_note",
  "follow_on_footnote",
  "editorial_note",
  "attachment_note",
...
const CHRONOLOGY_PATTERN =
  /\b(President['’]s Daily Diary|Daily Diary|from [0-9]{1,2}:[0-9]{2}|lasting from|until [0-9]{1,2}:[0-9]{2}|a\.m\.|p\.m\.|no indication as to when precisely|telephone calls took place|scheduled to take place|meeting took place)\b/i;

function usage() {
  console.error(
    "Usage: node scripts/audit-frus-chronology-usage.mjs --units extracted-units.json --registry registry.json [--checker-output output.json] [--target-volume ENTRY-ID] [--format text|json]"
  );
  process.exit(2);
}
...
function validateRegistry(registry) {
  ...
  if (registry.schema_version !== "frus-chronology-registry-v1") {
    errors.push("registry.schema_version: must be frus-chronology-registry-v1");
  }
...
function checkerChronologyDirectEdits(output) {
  const byUnit = new Map();
  if (!output || !Array.isArray(output.checks)) return byUnit;
  for (const check of output.checks) {
    if (!isPlainObject(check)) continue;
    const chronologySignal =
      ["chronology", "time_zone_chronology", "summit_public_event"].includes(check.category) ||
      ["chronology", "event_chronology", "time_zone_basis"].includes(check.evidence_request) ||
      /^FAS-(?:CHR|CHRON)-\d{3}$/.test(check.rule_id || "");
    if (!chronologySignal || !DIRECT_ACTIONS.has(check.recommended_action)) continue;
...
function usageStatus({ record, match, targetVolume }) {
  if (!String(record.verification_status || "").startsWith(VERIFIED_PREFIX)) return "needs_chronology_context";
  if (targetVolume && record.volume_id !== targetVolume) return "cross_volume_chronology";
...
function findingForStatus(status, record, match) {
  if (status === "approved") {
    return `Matched approved chronology phrase for ${record.document_id} ${record.chronology_type}.`;
  }
  if (status === "cross_volume_chronology") {
    return `Matched chronology language tied to ${record.volume_id}; confirm target volume before changing time, place, attendance, sequence, diary, schedule, or no-precise-time language.`;
  }
  if (status === "needs_chronology_context") {
    return `Matched ${match.matched_text}, but registry status is ${record.verification_status}; diary, schedule, call-log, or event-sequence basis is needed before direct edits.`;
  }
  return `Matched a chronology variant; review against the approved phrase ${JSON.stringify(record.approved_phrase)}.`;
}
...
function isChronologyLikeUnit(unit) {
  return CHRONOLOGY_UNIT_TYPES.has(unit.unit_type) && CHRONOLOGY_PATTERN.test(unitText(unit));
}
...
function auditChronology({ unitsDocument, registry, checkerOutput, targetVolume }) {
  const errors = [...validateUnits(unitsDocument), ...validateRegistry(registry), ...validateOutput(checkerOutput)];
  if (errors.length > 0) {
    return {
      schema_version: "frus-chronology-usage-audit-v1",
      status: "fail",
      errors,
      warnings: [],
      summary: {
        ...
        chronology_usages: 0,
        warnings: 0,
        unmatched_chronology_like_units: 0,
        direct_chronology_edit_conflicts: 0,
        by_usage_status: {},
        by_chronology_type: {}
      },
      usages: [],
  ...
  for (const unit of unitsDocument.units) {
    ...
    if (unitUsages.length === 0 && isChronologyLikeUnit(unit)) {
      unmatched += 1;
      const diagnostic = {
        unit_id: unit.unit_id,
        unit_type: unit.unit_type,
        location: unit.location || "",
        diagnostic_type: "unmatched_chronology_like_unit",
        finding: "Chronology, Daily Diary, meeting-time, call-time, or no-precise-time unit had no match in the supplied chronology registry.",
  ...
  for (const [unitId, checks] of checkerChronologyDirectEdits(checkerOutput).entries()) {
    ...
    for (const check of checks) {
      if (!directEditSupported(check, unitUsages)) {
        directConflicts.push({
          ...
          finding: "Direct chronology edit lacks a target-volume approved registry match.",
          required_action:
            "Downgrade to comment_only until diary, schedule, call-log, event-sequence, time-zone, place, attendance, and no-precise-time basis are supplied."
        });
      }
    }
  }
  ...
  const summary = {
    ...
    chronology_usages: usages.length,
    warnings: warnings.length,
    unmatched_chronology_like_units: unmatched,
    direct_chronology_edit_conflicts: directConflicts.length,
    by_usage_status: countBy(usages.map((item) => item.usage_status)),
    by_chronology_type: countBy(usages.map((item) => item.chronology_type))
  };
...
function renderText(result) {
  ...
  if (result.status === "fail") {
    lines.push(`FRUS chronology usage audit failed: ${result.errors.length} errors, ${result.warnings.length} warnings.`);
  } else {
    lines.push(`FRUS chronology usage audit ${result.status}: ${result.summary.chronology_usages} matches across ${result.summary.units_scanned} units.`);
    lines.push(`Variants/cross-volume/context warnings: ${result.summary.warnings}; unmatched chronology-like units: ${result.summary.unmatched_chronology_like_units}.`);
  }
...
try {
  ...
  console.error(`FRUS chronology usage audit failed: ${error.message}`);
  process.exit(1);
}
