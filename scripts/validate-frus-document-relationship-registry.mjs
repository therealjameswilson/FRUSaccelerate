...
const RELATIONSHIP_TYPES = new Set([
  "attached_but_not_printed_cross_reference",
  "attached_but_not_printed_no_target",
  "printed_as_document",
  "printed_in_volume",
  "see_document",
  "not_attached",
  "not_found_attached",
  "mixed_not_attached_and_printed_tabs",
  "unknown"
]);

const VERIFICATION_STATUSES = new Set([
  "verified_published_relationship",
  "verified_local_relationship",
  "needs_target_document",
  "needs_attachment_label",
...
function usage() {
  console.error("Usage: node scripts/validate-frus-document-relationship-registry.mjs --registry <document-relationship-registry.json|-> [--format json|text]");
  process.exit(2);
}
...
function validateRegistry(registry) {
  ...
  if (registry.schema_version !== "frus-document-relationship-registry-v1") {
    errors.push("$.schema_version: must be frus-document-relationship-registry-v1");
  }
  requireString(registry.document_relationship_registry_id, "$.document_relationship_registry_id", errors);
  ...
  registry.records.forEach((record, index) => {
    ...
    for (const key of [
      ...
      const optionalEmpty = ["target_document_id", "target_document_number", "target_label", "target_url"].includes(key);
      requireString(record[key], `${label}.${key}`, errors, { nonempty: !optionalEmpty });
    }
    if (ids.has(record.relationship_id)) {
      errors.push(`${label}.relationship_id: duplicate ${record.relationship_id}`);
    }
    ids.add(record.relationship_id);
    if (!RELATIONSHIP_TYPES.has(record.relationship_type)) {
      errors.push(`${label}.relationship_type: invalid value ${JSON.stringify(record.relationship_type)}`);
    }
    ...
    if (TARGET_REQUIRED_TYPES.has(record.relationship_type)) {
      for (const key of ["target_document_id", "target_document_number", "target_label", "target_url"]) {
        if (!String(record[key] || "").trim()) errors.push(`${label}.${key}: required for ${record.relationship_type}`);
      }
    }
    ...
    typeCounts[record.relationship_type] = (typeCounts[record.relationship_type] || 0) + 1;
    ...
    for (const form of forms) {
      const key = `${record.volume_id}\u0000${record.source_document_id}\u0000${record.source_unit_label}\u0000${record.relationship_type}\u0000${form}`;
      const existing = formMap.get(key);
      if (existing && existing !== record.relationship_id) {
        warnings.push(`${label}: relationship phrase ${JSON.stringify(form)} also appears in ${existing}`);
      } else {
        formMap.set(key, record.relationship_id);
      }
    }
  });
  ...
  return {
    ...
    summary: {
      ...
      by_relationship_type: typeCounts,
...
function renderText(result) {
  if (result.status === "pass" || result.status === "warning") {
    const lines = [
      `FRUS document-relationship registry validation ${result.status}: ${result.summary.records} records, ${result.warnings.length} warnings.`
    ];
    for (const warning of result.warnings) lines.push(`warning: ${warning}`);
    return `${lines.join("\n")}\n`;
  }
  const lines = [`FRUS document-relationship registry validation failed: ${result.errors.length} errors.`];
...
try {
  ...
  const result = {
    schema_version: "frus-document-relationship-registry-validation-v1",
  ...
  console.error(`FRUS document-relationship registry validation failed: ${error.message}`);
  process.exit(1);
}
